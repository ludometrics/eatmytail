...
function emtSetHighScore(value) {
	console.log("setHighScore: " + value + "  was " + emtGetHighScore());

	if (value > emtGetHighScore()) {
		createCookie("emtHighScore", value, 1461);
		document.getElementById('score-value').innerHTML = value;
	}
}

function emtGetHighScore() {
	var x = readCookie("emtHighScore");
	if (!x)
	{
		x = "0";
	}
				...
				var s = Crafty.e("2D, Canvas, Collision, tail")
											...
											.collision().onHit("snake", function() {
												if (segments.length > 1 && !scored) {
													...
													emtSetHighScore(this.segment * this.segment);
													scored = true;
												}
											});
				segments.push(s);
